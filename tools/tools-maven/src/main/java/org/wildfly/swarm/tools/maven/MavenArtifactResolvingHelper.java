...
public class MavenArtifactResolvingHelper implements ArtifactResolvingHelper {
    ...
    public MavenArtifactResolvingHelper(MavenHelper mavenHelper) {
        this.mavenHelper = mavenHelper.setResolvingHelper(this);
    ...
    public ArtifactSpec resolve(ArtifactSpec spec) {
        if (spec.file == null) {
            ...
            final LocalArtifactResult localResult = mavenHelper.repositorySystemSession().getLocalRepositoryManager()
                    .find(mavenHelper.repositorySystemSession(), new LocalArtifactRequest(artifact, this.remoteRepositories, null));
            if (localResult.isAvailable()) {
                spec.file = localResult.getFile();
            } else {
                try {
                    final ArtifactResult result = resolver.resolveArtifact(mavenHelper.repositorySystemSession(),
    ...
    private RemoteRepository buildRemoteRepository(final String id, final String url, final Authentication auth,
        ...
        final RemoteRepository mirror = mavenHelper.repositorySystemSession().getMirrorSelector().getMirror(repository);

        if (mirror != null) {
            final org.eclipse.aether.repository.Authentication mirrorAuth = mavenHelper.repositorySystemSession().getAuthenticationSelector()
        ...
        Proxy proxy = mavenHelper.repositorySystemSession().getProxySelector().getProxy(repository);
