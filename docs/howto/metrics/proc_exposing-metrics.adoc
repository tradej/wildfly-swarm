...
Exposing metrics to Prometheus--a monitoring solution--can be done in a configuration file in your {Thorntail}-based application.
Collect the metrics using annotations in your application source code.

In this example, you:

* Configure your application to collect metrics.
* View the data using Prometheus.
...
----
+
Prometheus expects metrics to be exposed at the `/metrics` endpoint of your application, which is a requirement of the MicroProfile Metrics specification.
It is also the default behavior of {Thorntail}-based applications.
--
...
. Annotate methods or classes with the metrics annotations that you want, for example:
+
--
[source,java]
----
